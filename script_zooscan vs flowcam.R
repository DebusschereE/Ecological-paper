...
#Data
t=read.csv("Cruise_data_2017_V9_complete_Flowcam.csv",h=T,row.names=1)
...
colnames(t)
#select dataframe geographic coordinates, zooplankton, all fcm data, flowcam data, chemtax, abiotic parameters, fluoroprobe
#
df=t[,c(3:4, 130:158,43:64)]
rownames(df)

#remove NA rows for missing values
df<-na.omit(df)##18 stations are remaining. All data available at these stations
#Zoo
colnames(df)
zoo=df[,c(32:53)]
